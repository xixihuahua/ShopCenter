/**
 * 
 */
package com.shop.Service.impl;

import java.util.List;

import com.shop.Dao.impl.GoodsDaoImpl;
import com.shop.Dao.impl.GoodsTypeDaoImpl;
import com.shop.Service.IGoodsTypeService;
import com.shop.entity.GoodsType;
import com.shop.entity.Goodsinfo;

/**
 * @author xixihuahua
 *
 */
public class GoodsTypeServiceImpl implements IGoodsTypeService {
	GoodsTypeDaoImpl goodsTypeDao = new GoodsTypeDaoImpl();
	/**
	 * 通过商品类型名查找商品类型
	 * @param goodsType_name
	 * @return
	 */
	 public GoodsType findGoodsTypeByTypeName(String goodsType_name) {
		return goodsTypeDao.findGoodsTypeByTypeName(goodsType_name);
	}
	 /**
	 * 通过商品类型id查找商品类型
	 * @param goodsType_id
	 * @return
	 */
	 public GoodsType findGoodsTypeByTypeId(Long goodsType_id) {
		return goodsTypeDao.findGoodsTypeByTypeId(goodsType_id);
	}
	/**
	 * 添加商品类型
	 * @param goodsType
	 * @return 
	 */
	public boolean addGoodsType(GoodsType goodstype) {
		return false;
	}
	
	/**
	 * 删除商品类型
	 * @param goodsType_id
	 * @return
	 */
	public boolean deleteGoodsType(Long goodsType_id) {
		return false;
	}
	/**
	 * 修改商品类型
	 * @param goodsType_id
	 * @return
	 */
	public boolean updateGoodsType(Long goodsType_id) {
		return false;
	}
	/**
	 * 通过商品类型找所有商品
	 */
	public List<Goodsinfo> findGoodsByType(Long goodsType_id) {
		// TODO Auto-generated method stub
		return goodsTypeDao.findGoodsByType(goodsType_id);
	}
	
	/**
	 * 分页查询 
	 * 通过商品类型名字  查找所有商品
	 * @param goodsType_name
	 * @param startRow
	 * @param endRow
	 * @return
	 */
	public List<Goodsinfo> SelectfindGoodsByType(String goodsType_name, int startRow,
			int endRow) {
		return goodsTypeDao.SelectfindGoodsByType(goodsType_name, startRow, endRow);
	}
	
	
	/**
	 * 通过商品类型名字  查找所有商品
	 * @param goodsType_id
	 * @return
	 */
	public List<Goodsinfo> findGoodsByTypeName(String goodsType_name) {
		return goodsTypeDao.findGoodsByTypeName(goodsType_name);
	}

}
